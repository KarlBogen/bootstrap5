...
if (xtc_db_num_rows($reviews_query)) {
  while ($reviews = xtc_db_fetch_array($reviews_query)) {
    ...
    $data_reviews[$reviews['reviews_rating']][] = array(
      ...
      'RATING_VOTE' => constant('BS5_AWIDSRATINGBREAKDOWN_LINK_' . $reviews['reviews_rating']),
...
$headline_1 = $counter . ' ' . constant('BS5_AWIDSRATINGBREAKDOWN_HEADLINE1');
$popup_smarty->assign('headline_1', $headline_1);
for ($i = 2; $i < 6; $i++) {
  $plus = isset($data_reviews[$i]) ? count($data_reviews[$i]) : 0;
  ${'headline_' . $i} = $plus . ' ' . sprintf(BS5_AWIDSRATINGBREAKDOWN_HEADLINE2, $i);
  $popup_smarty->assign('headline_' . $i, ${'headline_' . $i});
  $counter = $counter + $plus;
}
$headline_all = sprintf(BS5_AWIDSRATINGBREAKDOWN_HEADLINE3, $counter);
