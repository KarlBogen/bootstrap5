...
if (defined('BS5_CUSTOMERS_REMIND') && BS5_CUSTOMERS_REMIND == 'true') {
	...
	if (ENABLE_SSL == true && $request_type == 'NONSSL' && isset($_GET['view']) && $_GET['view'] == 'nonssl') {
		xtc_redirect(xtc_href_link(BS5_FILENAME_CUSTOMERS_REMIND, 'pID=' . (int) $_GET['pID'] . '&products_id=' . (int) $_GET['products_id'] . '&view=ssl', 'SSL'));
	}
	...
	if (isset($_SESSION['customer_id'])) {
		...
		$reg_query = xtc_db_query("SELECT * FROM " . TABLE_BS5_CUSTOMERS_REMIND . " WHERE customers_id =" . $customer_id . " AND products_id =" . $products_id);
		...
		if (isset($_POST['action']) && $_POST['action'] == 'add_remind' && $error === false) {
			...
			xtc_db_perform(TABLE_BS5_CUSTOMERS_REMIND, $sql_data_array);
			...
			if (!empty($registred)) {
				...
				$smarty->assign('CUSTOMERS_MAIL_INPUT', xtc_draw_input_field('customers_input_email', isset($_POST['customers_input_email']) ? xtc_db_prepare_input($_POST['customers_input_email']) : $_SESSION['customer_email_address'], 'id="email" class="form-control"'));
				$smarty->assign('CUSTOMERS_INPUT_ST', xtc_draw_input_field('customers_input_st', isset($_POST['customers_input_st']) ? xtc_db_prepare_input($_POST['customers_input_st']) : 1, 'id="qty" class="form-control"'));
