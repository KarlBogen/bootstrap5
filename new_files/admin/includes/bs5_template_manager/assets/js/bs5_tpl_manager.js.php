...
$(document).ready(function () {
	...
	if($('.error_message').length === 0){
	    var previewtheme = $('select[name="configuration[BS5_BOOTSTRAP_THEME]"] option:selected').text();
...
function checkAction(action) {
	...
	if (action == 'load_thememodel') {
		return confirmBs5("<?php echo BS5_LOAD_THEMEMODEL; ?>","",action);
	}
	if (action == 'load_thememodel2') {
		return confirmBs5("<?php echo BS5_LOAD_THEMEMODEL; ?>","",action);
	}
	if (action == 'copy') {
		return confirmBs5('<?php echo sprintf(BS5_COPY_2_TEMPLATE , '/templates/'.$bs5_conf["BS5_CURRENT_TEMPLATE"].'/'); ?>','',action);
	}
}
function confirmBs5(message, title, action) {
