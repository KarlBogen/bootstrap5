...
/* BOF Karl in Produktinfo Bilder tauschen */
    $(document).on('click', '.swap', function() {
        var btn = document.querySelector(".rightRs"),
        goto = $(this).data('goto');
        ResCarousel(btn, goto);
    });
/* EOF Karl in Produktinfo Bilder tauschen */
...
function ResCarouselBtn(sel) {
	var count = $(sel).find('.item').length;
	if (count < 5){
        var r = $(sel).width();
    	var itemWidth = $(sel).find('.item').width();
    	var ptn = (count * itemWidth) <= r;
        //console.log('r: '+r);
        //console.log('count * itemWidth: '+(count * itemWidth));
        //console.log('ptn: '+ptn);
    	$(sel).find('.resBtn').toggleClass("invisible", ptn);
    }
}
...
/* Karl wird für uns nicht benötigt
        for (var i = 0; i < 5; i++) {
            if (i == 0) {
                var styleCollector0 = ".ResSlid" + index + " .item {width: calc(" + 100 / itemsSplit[i] + "%)}";
            } else if (i == 1) {
                var styleCollector1 = ".ResSlid" + index + " .item {width: calc(" + 100 / itemsSplit[i] + "%)}";
            } else if (i == 2) {
                var styleCollector2 = ".ResSlid" + index + " .item {width: calc(" + 100 / itemsSplit[i] + "%)}";
            } else if (i == 3) {
                var styleCollector3 = ".ResSlid" + index + " .item {width: calc(" + 100 / itemsSplit[i] + "%)}";
            } else if (i == 4) {
                var styleCollector4 = ".ResSlid" + index + " .item {width: calc(" + 100 / itemsSplit[i] + "%)}";
            }
        }
*/
        ...
        //var it = r >= 1200 ? itemsSplit[3] : r >= 992 ? itemsSplit[2] : r >= 768 ? itemsSplit[1] : itemsSplit[0];
        //$(this).attr("data-itm", it);
        //var breaks = ["(max-width: 1399.98px)","(max-width: 991.98px)","(max-width: 767.98px)","(max-width: 575.98px)"];

/* Karl wird für uns nicht benötigt
        var styleCollector = "@media (max-width:575.98px){" + styleCollector0 + "}" +
            "@media (min-width:576px){" + styleCollector1 + "}" +
            "@media (min-width:768px){" + styleCollector2 + "}" +
            "@media (min-width:992px){" + styleCollector3 + "}" +
            "@media (min-width:1400px){" + styleCollector4 + "}";
...
function ResCarousel(Btn, goto = false) {
    //var t0 = performance.now();
