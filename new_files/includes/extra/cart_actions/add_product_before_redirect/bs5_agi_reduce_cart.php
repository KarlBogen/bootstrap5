  ...
  if (STOCK_CHECK == 'true' && STOCK_ALLOW_CHECKOUT != 'true' && $goto != FILENAME_SHOPPING_CART) {
    if(!$wishlist) {
      if(isset($_POST['products_id']) && is_numeric($_POST['products_id'])) {
        if(!isset($_SESSION['BS5_AGI_REDUCE_CART']))
          $_SESSION['BS5_AGI_REDUCE_CART'] = array();
        ...
        if($rest < $agi_qty) {
          $_SESSION['BS5_AGI_REDUCE_CART'][$agi_attributes_id_string] = array($agi_old_qty,$rest);
          if($rest > 0) {
            $cart_object->add_cart($agi_products_id, $rest, $agi_attributes, false);
            $messageStack->add_session('product_info', BS5_AGI_REDUCE_CART_MESSAGE_PINFO_REDUCED);
          }
          else {
            $cart_object->remove($agi_attributes_id_string);
            $messageStack->add_session('product_info', BS5_AGI_REDUCE_CART_MESSAGE_PINFO_OUT_OF_STOCK);
          }
        }
      }
    }
