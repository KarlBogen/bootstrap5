  ...
  if (STOCK_CHECK == 'true' && STOCK_ALLOW_CHECKOUT != 'true') {
    ...
    if ($rest < $products[$i]['quantity']) {
      ...
      if (!empty($products_query_result)) {
        ...
        $agi_products_price = $xtPrice->xtcGetPrice(
          $agi_prid,
          $format = false,
      ...
      $module_content[$i]['PRODUCTS_AVAILABLE_DATA'] = BS5_AGI_REDUCE_CART_SHOW_AVAILABLE == 'true' ? array('want' => $agi_old_qty, 'available' => $products[$i]['quantity']) : '';
    } elseif (!empty($_SESSION['BS5_AGI_REDUCE_CART'])) {
      if (array_key_exists($products[$i]['id'], $_SESSION['BS5_AGI_REDUCE_CART'])) {
        if ($_SESSION['BS5_AGI_REDUCE_CART'][$products[$i]['id']][1] > 0) {
          $_SESSION['any_stock_reduced'] = 1;
          $prod_mark_stock = '<span class="markProductOutOfStock">' . STOCK_MARK_PRODUCT_OUT_OF_STOCK . '</span>';
        } else {
        ...
        $module_content[$i]['PRODUCTS_NAME'] = $products[$i]['name'] . $prod_mark_stock;
        $module_content[$i]['PRODUCTS_QTY_REDUCED'] = true;
        $module_content[$i]['PRODUCTS_AVAILABLE_DATA'] = BS5_AGI_REDUCE_CART_SHOW_AVAILABLE == 'true' ? array('want' => $_SESSION['BS5_AGI_REDUCE_CART'][$products[$i]['id']][0], 'available' => $_SESSION['BS5_AGI_REDUCE_CART'][$products[$i]['id']][1]) : '';
        unset($_SESSION['BS5_AGI_REDUCE_CART'][$products[$i]['id']]);
      }
    }
  }
}
