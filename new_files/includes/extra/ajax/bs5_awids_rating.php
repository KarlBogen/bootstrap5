...
function bs5_awids_rating() {
	...
	$breakdown = array();
	$breakdown_content = '';
	$popup = constant('BS5_AWIDSRATINGBREAKDOWN_URL');
        ...
        $sum_reviews = (int)$full["result"];
			...
			$data = array (
				'BS5_AWIDS_PRODUCTS_RATINGDATA' => '',
				'BS5_AWIDS_PRODUCTS_SUM_REVIEWS' => 0,
				'BS5_AWIDS_PRODUCTS_AVERAGE' => 0,
				'BS5_AWIDS_PRODUCTS_AVERAGE_PERCENT' => 0,
			);
        ...
        if ($sum_reviews > 0) {
			...
			$data = array (
				'BS5_AWIDS_PRODUCTS_RATINGDATA' => $data_result,
		...
		//$rating = '<div class="ratings"><div class="empty-stars"></div><div class="full-stars" style="width:'.$data['BS5_AWIDS_PRODUCTS_AVERAGE_PERCENT'].'%"></div></div>&nbsp;('.$data['BS5_AWIDS_PRODUCTS_SUM_REVIEWS'].')';
		$rating = '<div class="ratings"><div class="fas empty-stars"></div><div class="fas full-stars" style="width:'.$data['BS5_AWIDS_PRODUCTS_AVERAGE_PERCENT'].'%"></div></div>&nbsp;('.$data['BS5_AWIDS_PRODUCTS_SUM_REVIEWS'].')';
		for ($i=1, $n=6; $i<$n; $i++) {
			$count = (isset($data['BS5_AWIDS_PRODUCTS_RATINGDATA'][$i]) && $data['BS5_AWIDS_PRODUCTS_RATINGDATA'][$i] > 0) ? $data['BS5_AWIDS_PRODUCTS_RATINGDATA'][$i] : 0;
			$percent = $data['BS5_AWIDS_PRODUCTS_SUM_REVIEWS'] == 0 ? 0 : number_format($count*100/$data['BS5_AWIDS_PRODUCTS_SUM_REVIEWS'], 0, '.', '');
			$sum = $count.' ('.$percent.'%)';
			$color = 'bg-'.$colors[$i-1];
			$stars = $i.'&nbsp;<i class="fas fa-star"></i>';
			$popup_link = xtc_href_link(BS5_FILENAME_POPUP_REVIEWS, 'products_id='.$pid.'&stars='.$i);
		...
		if (!empty($breakdown)) {
			$breakdown_content .= '<div class="avg_container bg-secondary-subtle border p-2 ' . $class . '">';
			...
			$breakdown_content .= '<div class="avg_bar">
										...
										<div class="d-flex">
											...
											<div class="avg_center flex-grow-1 px-1">
											...
											<div class="avg_right text-nowrap">
